...
public class StoneGeneratorDatabase {
    ...
    public void saveGenerators(Map<String, List<StoneGenerator>> map) {
        ...
        try {
            ...
            try (PreparedStatement deleteStmt = connection.prepareStatement(deleteSql);
                ...
                for (Map.Entry<String, List<StoneGenerator>> entry : map.entrySet()) {
                    String islandId = entry.getKey();
                    List<StoneGenerator> generators = entry.getValue();

                    if (islandId != null) {
                        deleteStmt.setString(1, islandId);
                    ...
                    for (StoneGenerator gen : generators) {
                        insertStmt.setString(1, islandId);
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    list.add(new StoneGenerator(targetId, location));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
